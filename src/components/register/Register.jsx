...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!passwordRegex.test(password)) {
      setError('Құпия сөзде кемінде 1 үлкен әріп, 1 кіші әріп, 1 сан және 1 символ болуы керек (6 таңбадан артық)');
      return;
    }
    ...
    try {
      const res = await axios.post('https://sevenproject-frontend-beckend-2.onrender.com/api/register', {
        username,
        email,
        password,
      });
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ flex: 1, backgroundColor: '#f25C0c', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        ...
        <h2 style={{ marginBottom: '20px', color: 'black', fontSize: '20px', fontWeight: 'bold', marginTop: '5px' }}>Тіркелу</h2>
        ...
        <form 
          ...
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', fontWeight: '500', marginBottom: '5px' }}>Аты</label>
            <input
              type="text"
              value={username}
              placeholder='Ayala'
          ...
          <button 
            type="submit" 
            style={{ backgroundColor: '#f45b0c', color: 'white', padding: '10px', border: 'none', borderRadius: '6px', width: '100%', fontWeight: 'bold' }}>
            Тіркелу
          </button>
